...
def getListName():
    path_source = r"E:\Pycharm projects\Auto_detect_bug\Training_Test\sourceFile_aspectj"
...
def getListDateBug():
    path_bug = "Training_Test\AspectJ_csv_trainingset.csv"
...
def computeRiAndFi(rMax, fMax,rMin, fMin):
    ...
    file['Ri'] = ri
    file['Fi'] = fi
    file.to_csv('Training_Test\InputForEnhanceCNN_label0.csv')

    file = pd.read_csv('Training_Test\InputForEnhanceCNN_label1.csv')
    data_ri = file['Ri']
    data_fi = file['Fi']
    ri = []
    fi = []
    ...
    file['Ri'] = ri
    file['Fi'] = fi
    file.to_csv('Training_Test\InputForEnhanceCNN_label1.csv')
